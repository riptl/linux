...
bool user_ibt_signal_enter(struct pt_regs *regs)
{
	/*
	 * Signal delivery is a (wacky) userspace indirect function call
	 * of the signal handler, so require the next instruction to be
	 * ENDBR if IBT is enabled.
	 */
...
void
user_ibt_sigreturn(struct pt_regs *regs, bool wait_endbr)
{
	struct fpu *fpu = x86_task_fpu(current);
	u64 msrval = 0;
